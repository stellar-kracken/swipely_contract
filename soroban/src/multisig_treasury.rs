...
impl MultiSigTreasuryContract {
    ...
    pub fn initialize(
        ...
        if initial_signers.len() < config.threshold as u32 {
        ...
        for i in 0..initial_signers.len() {
            ...
            env.storage().persistent().set(&DataKey::Signer(signer.clone()), &signer_data);
            env.storage().persistent().set(&DataKey::Role(signer), &role);
        }
        
        let signer_count = initial_signers.len() as u32;
    ...
    pub fn propose_transaction(
        ...
        if !env.storage().persistent().has(&DataKey::Signer(creator.clone())) {
        ...
        let transaction_count: u64 = env.storage().instance().get(&DataKey::TransactionCount).unwrap();
        ...
        let mut tx_count: u64 = env.storage().instance().get(&DataKey::TransactionCount).unwrap();
        tx_count += 1;
        ...
        env.storage().persistent().set(&DataKey::Transaction(nonce), &transaction);
        
        // Auto-add creator signature
        Self::sign_transaction_internal(&env, nonce, creator.clone());
        
        // Log the action
        Self::log_transaction_action(&env, nonce, String::from_str(&env, "Created"), creator, String::from_str(&env, "Transaction proposed"));
    ...
    pub fn sign_transaction(env: Env, transaction_id: u64, signer: Address) {
        ...
        let signer_data: SignerData = env.storage().persistent().get(&DataKey::Signer(signer.clone())).unwrap();
    ...
    pub fn execute_transaction(env: Env, transaction_id: u64, executor: Address) {
        ...
        if transaction.asset != Address::from_string(&String::from_str(&env, "native")) {
            if !env.storage().persistent().has(&DataKey::WhitelistedAsset(transaction.asset.clone())) {
        ...
        // Log execution
        Self::log_transaction_action(&env, transaction_id, String::from_str(&env, "Executed"), executor, String::from_str(&env, "Transaction executed successfully"));
    }
    ...
    pub fn cancel_transaction(env: Env, transaction_id: u64, canceller: Address) {
        ...
        Self::log_transaction_action(&env, transaction_id, String::from_str(&env, "Cancelled"), canceller, String::from_str(&env, "Transaction cancelled"));
    }
    ...
    pub fn add_signer(env: Env, new_signer: Address, role: Role) {
        ...
        env.storage().persistent().set(&DataKey::Signer(new_signer.clone()), &signer_data);
        env.storage().persistent().set(&DataKey::Role(new_signer), &role);
    ...
    pub fn whitelist_asset(env: Env, asset: Address, whitelister: Address) {
        let admin: Address = env.storage().instance().get(&DataKey::Admin).unwrap();
        admin.require_auth();
        
        env.storage().persistent().set(&DataKey::WhitelistedAsset(asset), &true);
    }
    ...
    pub fn remove_whitelisted_asset(env: Env, asset: Address) {
        ...
        env.storage().persistent().remove(&DataKey::WhitelistedAsset(asset));
    }

    /// Get transaction details
    pub fn get_transaction(env: Env, transaction_id: u64) -> Option<Transaction> {
        env.storage().persistent().get(&DataKey::Transaction(transaction_id))
    }

    /// Get signer data
    pub fn get_signer(env: Env, signer_address: Address) -> Option<SignerData> {
        env.storage().persistent().get(&DataKey::Signer(signer_address))
    }

    /// Get all active signers
    pub fn get_all_signers(env: Env) -> Vec<SignerData> {
        let mut signers: Vec<SignerData> = Vec::new(&env);
        let signer_count: u32 = env.storage().instance().get(&DataKey::SignerCount).unwrap();
    ...
    pub fn is_asset_whitelisted(env: Env, asset: Address) -> bool {
        env.storage().persistent().has(&DataKey::WhitelistedAsset(asset))
    }
    ...
    fn sign_transaction_internal(env: &Env, transaction_id: u64, signer: Address) {
        ...
        transaction.current_signatures = transaction.signers.len() as u32;
        ...
        // Log signature
        Self::log_transaction_action(env, transaction_id, String::from_str(env, "Signed"), signer, String::from_str(env, "Signature added"));
    }

    /// Internal function to log transaction actions
    fn log_transaction_action(env: &Env, transaction_id: u64, action: String, actor: Address, details: String) {
        ...
        let logs: Vec<TransactionLog> = env.storage()
            .persistent()
        ...
        env.storage().persistent().set(&DataKey::TransactionHistory(transaction_id), &updated_logs);
    }
}
