#![no_std]
// `create_escrow` mirrors the EscrowDeposit fields as a public contract
// entrypoint; splitting it would break the ABI. The lint fires on code
// generated by the `#[contractimpl]`/`#[contractclient]` macros, whose
// spans don't consistently inherit an `#[allow]` placed on the impl block,
